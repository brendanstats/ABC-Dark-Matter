...
#model function; param = [a,d,e, Ec, rlim, b, q, Jb, Vmax, rmax, alpha, beta, gamma]
...
def hE(x, v, param):
	...
	rs = rmax/2.16           # rmax=2.16*rs
        Ps = (Vmax/0.465)**2.0   # Vmax=0.465*sqrt(Ps)
        Pr = genphi(x, alpha, beta, gamma, Vmax, rmax)
...
def G(param, steps):
	...
	#vrarr = np.linspace(0, vmax, steps)
	#vtarr = np.linspace(0, vmax, steps)
	...
	while i <= steps:
		...
		j = 1
			...
			k = 1
				...
				#vm, vrm, vtm, fm = findmax2(param, xarr[i-1],xarr[i], 
				#	vrarr[j-1], vrarr[j], vtarr[k-1], vtarr[k], 5)
				#gijk = 1.1*fm
...
def ftheta(x, v, theta, param=[2.0, -5.3, 2.5, 0.16, 1.5, -9.0, 6.9, 0.086, 21.0, 1.5, 1,3,1]):

	a,d,e, Ec, rlim, b, q, Jb, Vmax, rmax, alpha, beta, gamma = param

        rs = rmax/2.16                   # rmax=2.16*rs
        Ps = (Vmax/0.465)**2.0           # Vmax=0.465*sqrt(Ps)
        Pr = genphi(x, alpha, beta, gamma, rmax, Vmax,rmax)
		#Ps * ( 1 - (np.log(1+x))/x )
	...
	xlim = rlim/rs
	Plim = genphi(xlim, alpha, beta, gamma, rmax, Vmax, rmax)
		#Ps * ( 1 - (np.log(1+xlim))/xlim )
	Ec   = Ec * Ps
	Jb   = Jb * rs * (Ps**0.5) #*0.086
...
def genphi(x, alpha = 1, beta = 3, gamma = 1, Vmax = 21.0, rmax = 1.5):
	...
	phi1 = integrate.quad(Integrand1, 0.0, x)[0]
	phi2 = integrate.quad(Integrand2, x, np.inf)[0]

	ans0 = Ps * ( 1 - (phi1/x + phi2) )
	'''
	x0 = 10**-10
...
def findmax (param = [2.0, -5.3, 2.5, 0.16, 1.5, -9.0, 6.9, 0.086, 21.0, 1.5,1,3,1]):
	'''
	flist = []
	dx    = 0.01
	dvr   = 0.01
...
def findmax1(param):
	...
	rs    = rmax/2.16
        xmax  = rlim/rs
	xi, xf = [10**-10, xmax] 
	vmax0 = vesc(xi, [rlim, Vmax, rmax, alpha, beta, gamma])
	vti, vtf = [0, vmax0]
	vri, vrf = [0, vmax0]
	steps = 20
	i = 0
	while i<10:
		xm, vrm, vtm, fm = findmax2(param, xi, xf, vri, vrf, vti, vtf, steps)
		delx = (xf-xi)/(1.*steps)
		delr = (vrf-vri)/(1.*steps)
		delt = (vtf-vti)/(1.*steps)

		xi = (xm - delx + abs(xm - delx) + 10**-10)/2.
		xf = xm + delx
		vri = (vrm - delr + abs(vrm - delr))/2.
		vrf = vrm + delr
		vti = (vtm - delt + abs(vtm - delt))/2.
		vtf = vtm + delt

		i = i + 1
		print 'findmax1 steps: ', i, xm, vrm, vtm, fm
	return xm, vrm, vtm, fm


def findmax2(param, xi,xf, vri, vrf, vti, vtf, steps = 10):
	xx = np.linspace(xi, xf, steps)
	vrr = np.linspace(vri, vrf, steps)
	vtt = np.linspace(vti, vtf, steps)
        ...
        x = 0.00001
        xm, vtm, vrm, fm = [0,0,0,0]
        for x in xx:  #loop through r/rs
                #vmax = vesc(x,[rlim, Vmax, rmax, alpha, beta, gamma])
                for vr in vrr:  #loop through radial velocity
                        #vtmax = (vmax * vmax - vr*vr)**0.5
                        for vt in vtt:
                                fvt = fprob(x, vr, vt, param)
                                if(fvt > fm):
...
def sample2( param = [2.0, -5.3, 2.5, 0.16, 1.5, -9.0, 6.9, 0.086, 21.0, 1.5, 1,3,1], steps = 10, samplesize = 3000):
	...
	errnum = 0
		...
		if (fi/gi > 1.0):
			errnum = errnum+1
			print num, 'Warning new: f/g>1', xi, vri, vti, fi, gi

                aux = aux + 1
	print errnum, samplesize
        print "acceptance rate: ", num/(aux+0.00001)
	return convert(samplelist, rs)
...
def sample3(param, steps, samplesize, resamplefactor = 1.5):
	a,d,e, Ec, rlim, b, q, Jb, Vmax, rmax, alpha, beta, gamma = param
	rs    = rmax/2.16
	xarr, vrarr, vtarr, Gx, Gxvr, Gxvrvt, pxvrvt = G(param, steps)
...
def sample3R(param, steps, samplesize, resamplefactor = 1.5):
	a,d,e, Ec, rlim, b, q, Jb, Vmax, rmax, alpha, beta, gamma = param
	rs    = rmax/2.16
	xarr, vrarr, vtarr, Gx, Gxvr, Gxvrvt, pxvrvt = G(param, steps)
...
def projecteddensity2(param=[2.0, -5.3, 2.5, 0.16, 1.5, -9.0, 6.9, 0.086, 21.0, 1.5, 1,3,1], dR = 0.02):
...
def projden(param=[2.0, -5.3, 2.5, 0.16, 1.5, -9.0, 6.9, 0.086, 21.0, 1.5, 1,3,1], dR = 0.02):
...
def rhortheta(r, param = [2.0, -5.3, 2.5, 0.16, 1.5, -9.0, 6.9, 0.086, 21.0, 1.5, 1,3,1]):
        a,d,e, Ec, rlim, b, q, Jb, Vmax, rmax, alpha, beta, gamma = param
        rs = rmax/2.16           # rmax=2.16*rs
...
def callosvsigprojdentheta(param = [2.0, -5.3, 2.5, 0.16, 1.5, -9.0, 6.9, 0.086, 21.0, 1.5,1,3,1], dR = 0.03):
	a,d,e, Ec, rlim, b, q, Jb, Vmax, rmax, alpha, beta, gamma = param
        rs = rmax/2.16           # rmax=2.16*rs
        Ps = (Vmax/0.465)**2.0   # Vmax=0.465*sqrt(Ps)
        losvsiglist = []
...
def callosvdisp(param=[2.0, -5.3, 2.5, 0.16, 1.5, -9.0, 6.9, 0.086, 21.0, 1.5, 1,3,1], dR = 0.02):
...
def callosvdisp2(param=[2.0, -5.3, 2.5, 0.16, 1.5, -9.0, 6.9, 0.086, 21.0, 1.5, 1,3,1], dR=.02, dx=.05, dv=0.1):
...
def rhosigrt2(param = [2.0, -5.3, 2.5, 0.16, 1.5, -9.0, 6.9, 0.086, 21.0, 1.5,1,3,1], dx = 0.05, dv = 0.1):
	a,d,e, Ec, rlim, b, q, Jb, Vmax, rmax, alpha, beta, gamma = param
        rs = rmax/2.16           # rmax=2.16*rs
        Ps = (Vmax/0.465)**2.0   # Vmax=0.465*sqrt(Ps)
